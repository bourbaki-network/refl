  ...
  def load(where: str) -> Optional['Package']:
    if path.exists(where) and path.isfile(where):
      with open(where) as f:
        ...
        origin: Origin = y["origin"] if "origin" in y else None
        options: Union[GitOptions, LocalOptions] = y["options"] if "options" in y else None
        ...
        dependencies: Optional[List['Package']] = y["dependencies"] if "dependencies" in y else None
